...
import Page from '../../../src/views/auth/blog';
...
const Index = ({ user, domaine, articles }) => {
    let next = null;
    let previous = null;
    if (articles && articles.current > 0) {
        previous = `${domaine}/blog?page=${articles.current - 1}`;
    }
    if (articles && articles.current < articles.max) {
        next = `${domaine}/blog?page=${articles.current + 1}`;
    }
    return (
        <Fragment>
            <Head
                ...
                url={articles ? `${domaine}/blog?page=${articles.current}` : "/"}
